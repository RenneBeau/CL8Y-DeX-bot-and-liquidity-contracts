...
//! `proxy` and `update_config` refuses to re-point them, so a deploying or
...
fn assert_proxy_canonical(
    ...
    match (canonical, proxy) {
        (Some(expected), Some(proxy)) if proxy.as_str() == expected => Ok(()),
        (Some(expected), _) => Err(ContractError::NonCanonicalAddress {
            field: "proxy",
            expected,
        }),
...
#[cfg(feature = "mainnet")]
pub fn assert_proxy_canonical_mainnet(proxy: Option<&Addr>) -> Result<(), ContractError> {
...
mod tests {
    ...
    #[test]
    fn proxy_pin_rejects_non_canonical_and_none() {
        let canonical = Some("terra1proxy");
        assert!(assert_proxy_canonical(canonical, Some(&addr("terra1proxy"))).is_ok());
        assert!(assert_proxy_canonical(canonical, Some(&addr("terra1fake"))).is_err());
        assert!(assert_proxy_canonical(canonical, None).is_err());
    }
    ...
    fn public_endpoints_use_the_config_constants() {
        ...
        assert_eq!(
            assert_proxy_canonical_mainnet(Some(&addr("anything"))).is_ok(),
