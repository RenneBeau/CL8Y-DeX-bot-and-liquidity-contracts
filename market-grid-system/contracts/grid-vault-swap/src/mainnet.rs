...
pub fn assert_proxy_canonical_mainnet(proxy: Option<&Addr>) -> Result<(), ContractError> {
    match proxy {
        Some(proxy) if proxy.as_str() == CANONICAL_SWAP_PROXY => Ok(()),
        _ => Err(ContractError::NonCanonicalAddress {
            field: "proxy",
...
mod tests {
    ...
    fn proxy_pin_rejects_non_canonical_and_none() {
        assert!(assert_proxy_canonical_mainnet(Some(&addr(CANONICAL_SWAP_PROXY))).is_ok());
        assert!(assert_proxy_canonical_mainnet(Some(&addr("terra1fake"))).is_err());
        assert!(assert_proxy_canonical_mainnet(None).is_err());
    }
}
