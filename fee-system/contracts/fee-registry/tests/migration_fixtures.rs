...
use cl8y_fee_registry::contract::{migrate, query};
use cl8y_fee_registry::msg::{ConfigResponse, MigrateMsg, QueryMsg};
...
fn wrong_name_and_newer_version_are_rejected_without_mutation() {
    for metadata in [
        ...
        br#"{"contract":"crates.io:cl8y-fee-registry","version":"99.0.0"}"#.as_slice(),
    ] {
        let mut deps = mock_dependencies();
